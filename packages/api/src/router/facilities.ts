...
import { FacilityQuery, BuildingQuery, BuildingnameQuery, FacilitiesQuery} from "@local/db/queries";
...
export const FacilityRouter ={
  ...
  byId: publicProcedure.input(z.object({id: z.number()})).query(({input, ctx}) => {
    return FacilityQuery.execute({id: input.id});
  }),
  byBuilding: publicProcedure.input(z.object({building: z.string()})).query(({input, ctx}) => {
    return BuildingQuery.execute({building: input.building});
  }),
  byBuildingName: publicProcedure.input(z.object({building: z.string()})).query(({input, ctx}) => {
